...
srun --nodes=1 --ntasks=1 --cpus-per-task=16 python EXTRACT_HPT.py # to extract the best hyperparameters in a csv file.
sleep 30
...
for leadtime in {02..10}; do
    ...
    srun --nodes=1 --ntasks=1 --cpus-per-task=16 python DL_PREDICT.py --lr $lr --bs $bs --lr_factor $LR_FACTOR --filters $FILTERS --mask_type $MASK_TYPE --HPT_path $HPT_PATH --leadtime "day$leadtime" --dropout $dropout &
    sleep 60
