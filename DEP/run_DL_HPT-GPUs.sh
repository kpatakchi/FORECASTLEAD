  ...
  for lr in 0.01 0.001 0.0001 0.00001; do
    for bs in 4 8 16 32; do
      for leadtime in {10..10}; do
        ...
        srun --nodes=1 --ntasks=1 --gres=gpu:4 python DL_TRAIN.py --lr $lr --bs $bs --lr_factor $LR_FACTOR --filters $FILTERS --mask_type $MASK_TYPE --HPT_path ${HPT_PATH} --leadtime day$leadtime --dropout $dropout &
        sleep 8 # to make sure all 576 jobs fit in 32 nodes over time.
      done
    done
  done
done
