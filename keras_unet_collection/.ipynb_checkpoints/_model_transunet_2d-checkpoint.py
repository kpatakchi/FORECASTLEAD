...
def transunet_2d_base(input_tensor, filter_num, stack_num_down=2, stack_num_up=2, 
    ...
    encode_height = input_height // 2**(depth_-1)
    encode_width = input_width // 2**(depth_-1)
    
    # number of patches
    num_patches = (encode_height // patch_size[0]) * (encode_width // patch_size[1])
    ...
    X = tf.reshape(X, (-1, encode_height, encode_width, embed_dim))
