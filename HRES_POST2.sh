...
while (( $(date -d "${current_date}" +%s) <= $(date -d "${end}" +%s) )); do
    ...
    cdo -O -b F64 --no_history -timcumsum -divc,1000. -setattribute,tp@units=m -chname,pr,tp -mergetime ${search_patterns[@]} $merged_output &>> $HRES_LOG/CDO_post_4.out
    cdo -O -b F64 --no_history -mulc,0 -seltimestep,1 -shifttime,-1hour -copy $merged_output $merged_output2 &>> $HRES_LOG/CDO_post_4.out
    cdo -O -b F64 --no_history -mergetime $merged_output2 $merged_output $merged_output3 &>> $HRES_LOG/CDO_post_4.out
    ...
    cdo -O -b F64 --no_history -seltimestep,2/19 -divc,1000. -setattribute,tp@units=m -chname,pr,tp -mergetime ${search_patterns[@]} $merged_output11 &>> $HRES_LOG/CDO_post_4.out
    ...
    cdo -O -b F64 --no_history -seltimestep,2/17 -divc,1000. -setattribute,tp@units=m -chname,pr,tp -mergetime ${search_patterns[@]} $merged_output111 &>> $HRES_LOG/CDO_post_4.out
